...
def run():
    ...
    masks = []
    for roi_name, proper_name in zip(rois_to_build, roi_proper_name, strict=False):
        temp = arrayclass.PatientMask(ct_arr, ss, roi_name, proper_name=proper_name)
        masks.append(temp)
    ...
    if "eortc_qol" in config.data.supplemental_data.scan():
        surveyfile = pd.read_csv(config.data.supplemental_data.eortc_qol)
        prepper.populate_surveys(surveyfile)

    if "clinical" in config.data.supplemental_data.scan():
        pc_file = pd.read_csv(config.data.supplemental_data.clinical, index_col=0)
