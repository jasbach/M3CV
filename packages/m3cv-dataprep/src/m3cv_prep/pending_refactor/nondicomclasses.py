...
class CompositeCondition(Condition):
    def __init__(self, left_condition, operator, right_condition):
        ...
        if self.operator == "&":
            ...
            category = (
                f"({self.left_condition.category} & {self.right_condition.category})"
            )
            value = False
        elif self.operator == "|":
...
class Survey:
    def __init__(
        self, df, time_col="eortc_qlqc30_35_timestamp", id_col="MRN", days_cutoff=90
    ):
    ...
    def calculate_time(self, ptinfo):
        ...
        self.data.drop(
            self.data[self.data[self.time_col] == "[not completed]"].index, inplace=True
        )
        ...
        if new_len != prev_len:
            print(f"Note: {prev_len - new_len} rows dropped due to missing time data.")
    ...
    def bin_surveys(self, dest_dir, fields=None):
        ...
        destpath = os.path.join(dest_dir, f"{self.cutoff}day_binned_surveys.csv")
        new_df.to_csv(destpath)

    def evaluate(self, patient, condition, timeframe, cutoff=90, mode="majority"):
...
class PatientInfo:
    def __init__(self, df, id_col="MRN", time_col="RT Completion Date"):
        ...
        includesRT = (
            self.data["Treatment Type"].astype(str).apply(lambda x: "rt" in x.lower())
        )
