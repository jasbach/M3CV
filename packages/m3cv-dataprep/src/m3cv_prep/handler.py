...
class Preprocessor:
    ...
    def erase(self, mode):
        if mode.lower() == "ct":
            ...
            raise ValueError("Unrecognized mode - accepts CT, Dose, Mask, or ALL")
    ...
    def enforce_compat(self):
        ...
        for cond, desc in zip(conditions, testnames, strict=False):
            if not cond:
                raise ValueError(f"Array compatibility test failed - {desc}")
        ...
        for subarr in ["ct", "mask", "dose"]:
            if getattr(self, subarr) is not None:
                getattr(self, subarr).zoom(
                    max_zoom_factor=maximum, seed=seed, zoom_factor=exact
                )
        ...
        for subarr in ["ct", "mask", "dose"]:
            if getattr(self, subarr) is not None:
                getattr(self, subarr).shift(
                    max_shift=maximum, seed=seed, pixelshift=exact
                )
        ...
        for subarr in ["ct", "mask", "dose"]:
            if getattr(self, subarr) is not None:
                getattr(self, subarr).rotate(
                    degree_range=maximum, seed=seed, degrees=exact
                )
    ...
    def save(
        ...
        if boxed is False:
            ...
            if maskcentered is True:
                ...
                # set centerpoint not in X/Y but align Z axis with parotids
                merged = sum(
                    [
                        maskentry.array
                        for maskentry in self.mask
                        if "parotid" in maskentry.proper_name
                    ]
                )
                ...
                center = (
                    center[0],
                    round(self.ct.array.shape[1] / 2),
                    round(self.ct.array.shape[2] / 2),
                )
            else:
                center = None
            data = [
        ...
        mask_names = [
            mask.proper_name if mask.proper_name is not None else mask.roi_name
        ...
        with h5py.File(fname, "a") as file:
            ...
            for mask, name in zip(mask_arrays, mask_names, strict=False):
            ...
            if hasattr(self, "pt_chars"):
                if "pt_chars" not in file.keys():
                    ...
                    file["pt_chars"].attrs["fields"] = np.array(
                        self.pt_char_fields, dtype="S"
                    )
            if hasattr(self, "surveys"):
                if "surveys" not in file.keys():
                    file.create_dataset("surveys", data=self.surveys.astype("S"))
                    file["surveys"].attrs["fields"] = np.array(
                        self.survey_fields, dtype="S"
                    )
