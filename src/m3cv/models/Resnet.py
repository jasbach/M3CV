...
def _shortcut3d(input, residual):
    ...
    stride_dim1 = math.ceil(input.shape[DIM1_AXIS] \
        / residual.shape[DIM1_AXIS])
    stride_dim2 = math.ceil(input.shape[DIM2_AXIS] \
        / residual.shape[DIM2_AXIS])
    stride_dim3 = math.ceil(input.shape[DIM3_AXIS] \
...
def _early_fusion_direct(filters, reg_factor, mode='concat'):
    def f(in_tensor,fuseto):
        while in_tensor.shape[-1] != 16:
            in_tensor = Dense(units=max(round(in_tensor.shape[-1]/2),16),
                              activation='relu', kernel_initializer='he_normal',
                              kernel_regularizer=l2(reg_factor))(in_tensor)
    ...
    def f(in_tensor,fuseto):
        ...
        while to3d.shape[2] < fuseto.shape[2]:
            ...
            to3d = Conv3D(
                ...
                kernel_initializer='he_normal', kernel_regularizer=l2(reg_factor)
                )(to3d)
            to3d = Conv3D(
                filters, (3,3,3), strides=(1,1,1), padding='same',
                kernel_initializer='he_normal', kernel_regularizer=l2(reg_factor)
...
class Resnet3DBuilder(object):
    ...
    def build(input_shape, num_outputs, block_fn, repetitions, reg_factor,
        ...
        if input_shape[0] < 40:
            ...
            k_s = 7
            st = 2
        conv1 = _conv_bn_relu3D(filters=basefilters, kernel_size=(k_s, k_s, k_s),
                                strides=(st, st, st),
    ...
    def build_resnet_18(input_shape, num_outputs, reg_factor=1e-4, fusions={},
        ...
        return Resnet3DBuilder.build(input_shape, num_outputs, basic_block,
                                     ...
                                     fusions=fusions,basefilters=basefilters)

    @staticmethod
    def build_resnet_34(input_shape, num_outputs, reg_factor=1e-4, fusions={},
                        basefilters=64):
        """Build resnet 34."""
        return Resnet3DBuilder.build(input_shape, num_outputs, basic_block,
    ...
    @staticmethod
    def build_resnet_50(input_shape, num_outputs, reg_factor=1e-4, fusions={},
                        basefilters=64):
        """Build resnet 50."""
    ...
    @staticmethod
    def build_resnet_101(input_shape, num_outputs, reg_factor=1e-4, fusions={},
                        basefilters=64):
        """Build resnet 101."""
        return Resnet3DBuilder.build(input_shape, num_outputs, bottleneck,
    ...
    @staticmethod
    def build_resnet_152(input_shape, num_outputs, reg_factor=1e-4, fusions={},
                        basefilters=64):
        """Build resnet 152."""
        return Resnet3DBuilder.build(input_shape, num_outputs, bottleneck,
