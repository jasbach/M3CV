...
from m3cv.models.losses import bce_dice_loss, dice_coef
...
def get_dice_sum(predictions, test_labels, thresholding=False):
    ...
    dicetotal = (2.0 * intersectionsum + smooth) / (denominatorsum + smooth)
    return dicetotal


def get_unet(
    image_size, num_channels=1, lr=1e-5
):  # have adjusted filter size numbers to account for image size of 256
    # assert image_size == 256

    inputs = Input(
        (image_size, image_size, num_channels)
    )  # third number is the number of channels - configured for single W/L
    conv1 = Conv2D(
    ...
    model.compile(
        ...
        loss=bce_dice_loss,
        metrics=[dice_coef],
    )  # loss options: dice_coef_loss, bce_dice_loss, weighted_dice_loss, weighted_bce_loss
