...
def rotate(original, degree_range=15, seed=None, degrees=None):
    ...
    for channel in range(3):
        ...
        if channel == 2:
            subarray = np.round(subarray)  # return mask to binary
...
def shift(original, max_shift=0.2, seed=None, pixelshift=None):
    ...
    for channel in range(3):
        ...
        subarray = scipy_mods.shift(
            subarray, shift=shiftspec, mode="constant", cval=voidval
        )
        array[..., channel] = subarray
...
def zoom(original, max_zoom_factor=0.2, seed=None, zoom_factor=None):
    ...
    for channel in range(3):
        ...
        if zoom_factor < 1.0:
            ...
            subarray = np.pad(
                subarray, pad_width=pad_spec, mode="constant", constant_values=voidval
            )
        array[..., channel] = subarray
...
def downsample(array, factor=0.5, axis=[0, 1, 2]):
    is_binary = {}
    for channel in range(array.shape[-1]):
        if len(np.unique(array[..., channel])) == 2:
            is_binary[channel] = True
        else:
            is_binary[channel] = False
    zoom_map = [1] * array.ndim
    if not isinstance(axis, list):
        axis = [axis]
    for i in axis:
        zoom_map[i] = factor
    downsampled = scipy_mods.zoom(array, zoom=zoom_map)
    for channel, binary in is_binary.items():
        if binary:
            downsampled[..., channel] = np.round(downsampled[..., channel])
    return downsampled
