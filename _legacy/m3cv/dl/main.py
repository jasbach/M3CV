...
from m3cv.dl.io import Augmenter, DataLoader, Handler
...
"""
    ...
    augment_scheme = {op: getattr(augment_scheme, op) for op in augment_scheme.scan()}
    ...
    train_dataset = tf.data.Dataset.from_generator(
        handler, output_signature=handler.loader.output_sig
    )
    ...
    if build_args.fusion is True:
        if build_args.fusion_point == "early":
            ...
            fusepoint = "late"
        fusion_plan = {fusepoint: handler.supp_vector_len}
    else:
        fusion_plan = {}
    ...
    if config.model.name == "resnet-34":
        ...
        raise Exception("Unrecognized model argument, defaulting to ResNet-18")
    ...
    if build_args.optimizer == "adam":
        optim = keras.optimizers.Adam(learning_rate=build_args.learning_rate)
    elif build_args.optimizer == "sgd":
        optim = keras.optimizers.SGD(learning_rate=build_args.learning_rate)
    metrics = []
    ...
    model.compile(
        optimizer=optim, loss=keras.losses.BinaryCrossentropy(), metrics=metrics
    )
    ...
    fitargs = {
        ...
        "class_weight": {0: train_args.class_weight[0], 1: train_args.class_weight[1]},
    }
    # === Run model ===
    ...
    with tf.device("/CPU:0"):
        ...
        model.save(os.path.join(config.model.artifact_output, "full_final_model.h5"))
    ...
    report = classification_report(config.model.evaluation, testY, preds)
    print(report)
    with open(os.path.join(config.model.artifact_output, "report.json"), "w+") as f:
        json.dump(report, f)
