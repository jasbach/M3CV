...
def gen_inputs(config, labels, ptchars, training=True, verbose=False):
    ...
    * batch_size : int, optional
    ...
    if class_balance and any((not shuffle, not batch_size)):
        raise ValueError("Class balance without shuffle and batch_size not supported")
    elif class_balance:
        pos = []
        neg = []
    ...
    for i in range(epochs):
        if shuffle is False:
            ...
            if not class_balance:
                ...
                temp += list(
                    np.random.choice(pos, size=small_class_size, replace=False)
                )
                temp += list(
                    np.random.choice(neg, size=small_class_size, replace=False)
                )
                ...
                raise ValueError("Unrecognized value provided for class_balance arg")
    ...
    for patientID in patientlist:
        if verbose is True:
            print(f"Loading data for {patientID}")
        ...
        try:
            ...
            print(f"Review patient {patientID}")
            print(e)
            continue
        ...
        X = prep_inputs(
            img, expanded_dose, masks=[par_l, par_r], config=config, training=training
        )
...
def prep_inputs(img, dose, masks, config, training=True):
    """
    ...
    * augment : bool, optional
    ...
    ipsi_contra = config.getboolean("data_settings", "ipsi_contra", fallback=False)
    ...
    if wl:
        ...
        img = window_level(img, window=wl_window, level=wl_level, normalize=normalize)
    ...
    for mask in masks:
        ...
        if augment is True and training is True:
            ...
            if np.random.random() > 0.5:
                ...
                init_array = rotation(init_array, seed, degree_range=rotate_range)
            if np.random.random() > 0.5:
                seed = np.random.random()
                # zoom is way faster on 3D arrays than 4D, so we'll
                # deconstruct back to 3D components then re-stack
                templist = [init_array[..., i] for i in range(init_array.shape[-1])]
    ...
    if ipsi_contra:
        if len(output) == 2:
            ...
            raise Exception(
                f"Number of output arrays {len(output)}"
...
def dose_expand(img, dose, im_info, dose_info):
    ...
    if y_diff >= 0:
        y_dest = range(y_diff, min(y_diff + dose.shape[1], dose_arr.shape[1]))
    ...
    if x_diff >= 0:
        x_dest = range(x_diff, min(x_diff + dose.shape[2], dose_arr.shape[2]))
...
def rotation(array, seed=None, degree_range=15):
    ...
    array = rotate(array, angle=howmuch, axes=(1, 2), reshape=False, mode="nearest")
    return array
...
def get_unique_values(df, delimiter=None):
    ...
    for col in df.columns:
        uniqs = list(df[col].astype(str).unique())
...
def rebuild_mask(f, key):
    ...
    for sl in slice_nums:
        ...
        sparse = coo_matrix(
            (np.ones_like(cols), (rows, cols)), shape=f["ct"][...].shape[1:], dtype=int
        )
        dense[sl, ...] = sparse.todense()
    return dense


def stats_continuous(s):
    s = s.astype(float)
    s = s.dropna()
    mean = s.mean()
    q75, q25 = np.percentile(s, [75, 25])
    iqr = q75 - q25
    multi_index = pd.MultiIndex.from_tuples([(s.name, "Mean"), (s.name, "IQR")])
    return pd.Series([mean, iqr], index=multi_index)


def stats_categorical(s):
    baseline = s.value_counts()
    mi_tuples = []
    for val in baseline.index:
        mi_tuples.append((s.name, val))
    mi = pd.MultiIndex.from_tuples(mi_tuples)
    return pd.Series(baseline.values, index=mi)
